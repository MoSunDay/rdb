...
async fn session_timeout_sweeps_silent_member_to_rebalance() {
    ...
    assert_eq!(
        heartbeat_v0(&mut sock, 5, "g1", 1, &id).await,
    ...
    assert!(
        r.generation >= 2,
        "the Empty transition bumped the generation"
    );
    ...
    assert_eq!(
        commit_v2(&mut sock, 11, "g1", gen2, &id2, "t", 0, 3).await,
        errors::NONE
    );
    // The swept member fails the MEMBERSHIP check first; the surviving
    // member at a stale generation fails the generation check.
    assert_eq!(
        commit_v2(&mut sock, 12, "g1", 1, &id, "t", 0, 9).await,
        errors::UNKNOWN_MEMBER_ID
    );
    assert_eq!(
        commit_v2(&mut sock, 13, "g1", gen2 - 1, &id2, "t", 0, 9).await,
...
async fn restart_wipes_membership_but_keeps_offsets() {
    ...
    assert_eq!(err, errors::NONE);
    assert_eq!(
        commit_v2(&mut sock, 4, "g1", 1, &id, "t", 0, 7).await,
        errors::NONE
    );
    ...
    assert_eq!(
        r.generation, 3,
        "Empty bumped to 2, the rejoin barrier to 3"
    );
    let (err, _) = sync_v1(&mut sock, 7, "g1", &id, 3, &[(id.as_str(), b"[t0]")]).await;
    assert_eq!(err, errors::NONE);
    assert_eq!(
        commit_v2(&mut sock, 8, "g1", 3, &id, "t", 0, 7).await,
        errors::NONE
    );
    ...
    assert_eq!(
        fetch_offset_v0(&mut sock, 11, "g1", "t", 0).await,
        7,
        "offsets are durable"
    );
    ...
    assert_eq!(
        commit_v2(&mut sock, 12, "g1", 2, &id, "t", 0, 9).await,
        errors::ILLEGAL_GENERATION
    );
    ...
    assert_eq!(
        commit_v2(&mut sock, 17, "g1", 4, &id2, "t", 0, 8).await,
        errors::NONE
    );
    // The old member id no longer exists: fenced by the runtime layer.
    assert_eq!(
        commit_v2(&mut sock, 18, "g1", 4, &id, "t", 0, 9).await,
        errors::UNKNOWN_MEMBER_ID
    );
