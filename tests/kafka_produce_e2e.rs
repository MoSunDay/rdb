...
async fn list_offsets(
    ...
    let payload = kafka_round(
        sock,
        &kafka_req(API_LIST_OFFSETS, version, corr, false, &body),
    )
    .await;
    let mut r = Reader::new(&payload);
...
/// attributes..end).
fn set_attributes(batch: &mut [u8], attr: i16) {
...
async fn produce_appends_and_listoffsets_answer() {
    ...
    assert_eq!(
        produce_out(&body),
        (0, 0, 1, -1),
        "base_offset = pre-append len (1 xadd)"
    );
    ...
    assert!(
        got[2].2 > 1_000_000_000_000,
        "arrival-clock ms timestamp: {got:?}"
    );
    ...
    assert_eq!(
        r.i32(),
        Some(300),
        "next response is ApiVersions, not a produce ack"
    );
    let xlen = resp_one_shot(&resp, &[b"XLEN", b"t1/q0"]).await;
    assert!(
        contains_bytes(&xlen, b":5\r\n"),
