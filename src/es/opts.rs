...
use super::dsl::{parse_query, err, DslError, KnnPlan, SortDir, SortKey, SourceFilter};
...
pub(super) fn parse_knn(v: &Value) -> Result<KnnPlan, DslError> {
    ...
    let field = match obj.get("field") {
        ...
        _ => return Err(err(400, "parsing_exception", "[knn] requires a string field")),
    };
        ...
        Some(Value::Array(items)) => {
            ...
            for it in items {
                let Some(x) = it.as_f64().filter(|x| x.is_finite()) else {
                    return Err(err(400, "parsing_exception", "[knn] query_vector must be finite numbers"));
                };
                out.push(x);
            }
            out
        }
        _ => return Err(err(400, "parsing_exception", "[knn] requires a query_vector array")),
    };
    if vector.is_empty() {
        return Err(err(400, "parsing_exception", "[knn] query_vector must not be empty"));
    }
    ...
    let default_cands = (k * 10).max(100).min(10_000);
    ...
    Ok(KnnPlan { field, vector, k, num_candidates, filter })
}
        ...
        match it {
            ...
            Value::Object(obj) => {
                let Some((name, cfg)) = obj.iter().next() else {
                    return Err(err(400, "parsing_exception", "[sort] entry cannot be empty"));
                };
                let dir = match cfg {
                    Value::Object(b) => match b.get("order") {
                        ...
                        _ => return Err(err(400, "parsing_exception", "[sort] order must be 'asc' or 'desc'")),
                    },
                    _ => default_order(name),
                };
                out.push(sort_key_named(name, dir)?);
            }
            _ => return Err(err(400, "parsing_exception", "[sort] entries must be strings or objects")),
        }
    }
    Ok(out)
}
...
fn sort_key_named(name: &str, dir: SortDir) -> Result<SortKey, DslError> {
    match name {
        ...
        "_doc" => Ok(SortKey::Doc),
        _ => Ok(SortKey::Field { field: name.to_string(), dir }),
    }
}
...
pub(super) fn parse_source(v: Option<&Value>) -> Result<SourceFilter, DslError> {
    ...
    match v {
        Value::Bool(false) => Ok(SourceFilter { disabled: true, ..SourceFilter::default() }),
        Value::Bool(true) | Value::Null => Ok(SourceFilter::default()),
        Value::String(s) => Ok(SourceFilter { includes: vec![s.clone()], ..SourceFilter::default() }),
        Value::Array(a) => {
            let includes = strings(a, "_source")?;
            Ok(SourceFilter { includes, ..SourceFilter::default() })
        }
        Value::Object(obj) => {
            ...
            for (keys, out) in [(["includes", "include"], &mut includes), (["excludes", "exclude"], &mut excludes)] {
                for key in keys {
            ...
            Ok(SourceFilter { includes, excludes, ..SourceFilter::default() })
        }
        _ => Err(err(400, "parsing_exception", "[_source] must be a boolean, array or object")),
    }
}
...
fn patterns(v: Option<&Value>) -> Result<Vec<String>, DslError> {
    match v {
        ...
        Some(_) => Err(err(400, "parsing_exception", "[_source] patterns must be strings or arrays")),
    }
}
...
fn strings(a: &[Value], key: &str) -> Result<Vec<String>, DslError> {
    a.iter()
        .map(|v| {
            v.as_str()
                .map(str::to_string)
                .ok_or_else(|| err(400, "parsing_exception", &format!("[{key}] entries must be strings")))
        })
        .collect()
}

/// from/size as non-negative integers (missing key -> default).
pub(super) fn uint_of(obj: &Map<String, Value>, key: &str, default: usize) -> Result<usize, DslError> {
    match obj.get(key) {
        None | Some(Value::Null) => Ok(default),
        Some(v) => v
            .as_u64()
            .map(|x| x as usize)
            .ok_or_else(|| err(400, "parsing_exception", &format!("[{key}] must be a non-negative integer"))),
    }
}
