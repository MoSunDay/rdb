...
- `lite/`：Lite Mode（RocketMQ 风格父主题 + 动态队列）。`mod.rs` 运行时与装配、`model.rs` 主题名/物理布局（slot 前缀取父主题名）、`select.rs` 队列挑选（round_robin/hash/least_backlog）、`append.rs` xadd/xrange/xtrim/xdel/xidle、`read.rs` xlen/xread/xreadgroup（多流 STREAMS 配平）、`park_wait.rs` 阻塞读 park/唤醒（read 拆出）、`ack.rs` xack（同步持久化组水位）、`entries.rs` 条目扫描公共件、`group.rs` xgroup（含 CREATECONSUMER/DELCONSUMER）、`offset.rs` 组水位内存缓存 + 200ms 刷盘（kind-0x0E）、`pel.rs` PEL 行持久化（kind-0x0F，投递同步落盘，kill -9 后 at-least-once 重投）、`pending.rs` xpending（摘要+范围）、`claim.rs` xclaim、`autoclaim.rs` xautoclaim（claim 拆出）、`info.rs` xinfo（含 CONSUMERS）/xpick；空闲 TTL 复用统一过期信封，到期整流回收。stream 类即由此定形：不做 Redis Streams 全仿真，语义模型为 RocketMQ 主题/队列；`ds/codec.rs` 的 `KIND_STREAM_PEND 0x0F` 窗口由 `pel.rs` 落盘 PEL 行与消费者名（投递同步写，kill -9 后 at-least-once 重投）；组已提交水位仍走 kind-0x0E 记录。
- `rcache/`：openraft 控制面。
