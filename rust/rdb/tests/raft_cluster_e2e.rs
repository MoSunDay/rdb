...
async fn start_node(bootstrap: bool) -> TestNode {
    ...
    tokio::spawn(async move {
        if let Err(e) = http::serve_on(listener, raft_http, kv_http, TOKEN.into()).await {
...
async fn join_replicate_get_depart() {
    ...
    // /join with a wrong token replies "ok" (Go bug parity) but logs.
    let bad = format!("/join?peerAddress={}&raft-token=bad", n2.tcp_addr);
    assert_eq!(http_body(&n1, &bad).await, "ok");
