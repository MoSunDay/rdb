...
## Overview
- rdb 是支持 Redis Cluster 协议的可持久化 KV 存储。**Go 原实现已归档至 `archive/go/`（git mv 保留历史，不再维护）；Rust 实现 `rust/` 为唯一在维护实现**，见 [agents/rust/index.md](./agents/rust/index.md)。
...
- Rust 实现的架构/行为差异见 `rust/COMPAT.md`。
