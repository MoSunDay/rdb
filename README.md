...
- **数据迁移**：`migrate task <slot> <src> <dst>` 单命令编排无损 slot 迁移（redis-cli
  `--cluster reshard` 协议兼容：SETSLOT MIGRATING/IMPORTING/NODE、ASKING、GETKEYSINSLOT、
  MIGRATE/DUMP/RESTORE 传输），`migrate list` 返回任务 JSON；slot 归属经 raft
  `slot_owner_map` 收敛。
